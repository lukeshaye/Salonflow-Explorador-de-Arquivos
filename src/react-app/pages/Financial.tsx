...
interface FinancialFormData {
    description: string;
    amount: number; // O usuário insere em reais (ex: 10.50)
...
export default function Financial() {
  ...
  const onSubmit = async (formData: FinancialFormData) => {
    ...
    const entryData = {
      ...formData,
      amount: Math.round(formData.amount * 100), // Converte reais para cêntimos
    };
  ...
  const handleEditEntry = (entry: FinancialEntryType) => {
    ...
    reset({
      ...entry,
      // CORREÇÃO CRÍTICA: Converte de cêntimos para reais ao preencher o formulário.
